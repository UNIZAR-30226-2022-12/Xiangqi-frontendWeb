            ...
            <section class="p-4">
                ...
                <div class="w-full">
                    <div class="relative w-20rem m-auto md:ml-0">
                        <InputText placeholder="Nombre de usuario" class="w-16rem h-3rem pl-4" style="border-style: none; border-top-left-radius: 1rem; border-bottom-left-radius: 1rem; border-top-right-radius: 0; border-bottom-right-radius: 0; box-shadow: 0 3px 1px -2px rgba(0, 0, 0, 0.2), 0 2px 2px 0 rgba(0, 0, 0, 0.14), 0 1px 5px 0 rgba(0, 0, 0, 0.12);" v-model="searchFriendField"/>
                        <!--On click enviar evento-->
                        <Button v-if="this.showResults && this.show" v-on:click="clear()" class="border-circle p-button surface-400 border-400 border-3 mr-2 z-5 absolute" style="right: 4rem; width: 1.4rem; height: 1.25rem; margin-top: 0.8rem" icon="pi pi-times" iconPos="left" />
